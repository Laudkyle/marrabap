...
BEGIN
    ...
    -- Update customer total sale due (only for credit sales)
    UPDATE customers 
    SET total_sale_due = total_sale_due + NEW.total_price
    ...
    -- Get the last inserted journal entry id
    ...
    SELECT 
        ...
        0,
        NEW.total_price
    FROM last_journal, revenue_account;

    -- Handle the debit side (Cash or Accounts Receivable)
    INSERT INTO journal_entry_lines (journal_entry_id, account_id, debit, credit)
    SELECT 
        (SELECT id FROM journal_entries ORDER BY id DESC LIMIT 1),
        CASE 
